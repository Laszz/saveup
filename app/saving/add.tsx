...
export default function AddSaving(){
  const { goals, addSaving, settings } = useApp();
  ...
  const submit=async()=>{
    ...
    try{
      await addSaving(selected, amount, note, dateObj.toISOString());
      setSnack(tt('tabunganDitambahkan'));
  ...
  if(goals.length===0) return (
    <>
      ...
      <View style={[styles.wrap, { backgroundColor:p.bg, paddingTop:insets.top, padding: Spacing.sm, gap:12, justifyContent:'center', alignItems:'center' }]}>
        ...
        <Text style={{ color:p.subText, textAlign:'center' }}>{tt('buatGoalDulu')}</Text>
        <Button mode="contained" onPress={()=>router.push('/goal/form')} buttonColor={p.primary} textColor="#FFF">{tt('buatGoal')}</Button>
      </View>
    </>
  );
      ...
      <KeyboardAvoidingView style={{ flex:1, backgroundColor:p.bg }} behavior={Platform.OS==='ios'?'padding':'padding'} keyboardVerticalOffset={insets.top}>
        <View style={[styles.wrap, { backgroundColor:p.bg, paddingTop:insets.top }]}>
          <View style={[styles.header, { backgroundColor:p.card, borderColor:p.border }]}>
            <Text variant="titleMedium" style={[styles.headerTitle, { color:p.text, textAlign:'center' }]}>{tt('tambahTabungan')}</Text>
          </View>
          <ScrollView contentContainerStyle={{padding: Spacing.sm, gap:16, paddingBottom: insets.bottom + 24}} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets keyboardDismissMode="interactive">
            {goal && (
              <Card style={[styles.goalCard, { backgroundColor:p.primary+'12', borderColor:p.primary }]}>
                <Card.Content style={{ flexDirection:'row', alignItems:'center', gap:12 }}>
                  <View style={[styles.goalIcon, { backgroundColor:p.primary+'18' }]}><Icon source={goal.icon||'target'} size={20} color={p.primary} /></View>
                  <View style={{ flex:1 }}>
                    <Text style={[styles.goalName, { color:p.text }]}>{goal.name}</Text>
                    <Text style={[styles.goalSub, { color:p.subText }]}>{formatIDR(goal.currentAmount||0)} / {formatIDR(goal.targetAmount)}</Text>
                  </View>
                </Card.Content>
              </Card>
            )}
            ...
            <Button mode="contained" onPress={submit} loading={submitting} disabled={submitting} style={styles.btn} contentStyle={{ height:48 }} buttonColor={p.primary} textColor="#FFF">{tt('simpan')}</Button>
